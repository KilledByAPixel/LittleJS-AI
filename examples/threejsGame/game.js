/*
    LittleJS Three.js Platformer Demo
...
// three.js module, loaded from CDN at the top of gameInit
let THREE;
...
// camera constants
let cameraOffset;      // fixed follow offset, set in gameInit after three.js loads
...
class PlatformerObject extends ThreeJSObject
{
...
class Player extends PlatformerObject
{
    ...
    update()
    {
        ...
        this.velocity = this.velocity.add(keyDirection().scale(.015));

        // jump with space when standing on something
        if (keyWasPressed('Space') && this.isOnGround())
...
{
    // load three.js and setup the plugin with a free camera for the chase cam
    THREE = await import(threeModuleURL);
    new ThreeJSPlugin(THREE);
    threeJS.cameraAlign2D = false;  // this demo drives the camera itself
    threeJS.camera.up.set(0, 0, 1); // littlejs z is up
    cameraOffset = new THREE.Vector3(0, -9, 6);
...
function gameRender()
{
    // the three.js scene is the whole visual, nothing to draw behind objects
}
...
function gameRenderPost()
{
    ...
    const pos = vec2(mainCanvasSize.x/2, 45);
    const done = coinsCollected == coinsTotal ? ' — You got them all!' : '';
    drawTextScreen(`Coins: ${coinsCollected} / ${coinsTotal}${done}`, pos, 35);
    drawTextScreen('Arrows/WASD run — Space jump', pos.add(vec2(0, 35)), 20);
}
